...
const HEADER_MAP = {
  ...
  'dealstage': 'dealStage',
  'stage': 'dealStage',
  // Spend (manual column Omar will add)
  ...
  // Comments (ignored but mapped to avoid noise)
...
// Lead Quality column values — only "Relevant" counts as true
function parseRelevant(raw) {
  if (!raw) return false;
  return raw.trim().toLowerCase() === 'relevant';
}
...
export function computeFunnel(leads) {
  ...
  const contacted = leads.filter(l => {
    ...
    return s && s !== '' && !/^(not contacted|pending)$/i.test(s);
  }).length;
