...
const HEADER_MAP = {
  ...
  'quality': 'relevant',
  'qualified': 'relevant',
  // Deal Stage — the MQR sheet column is "Deal Stage (If relevant)"
  'deal stage': 'dealStage',
  'deal stage (if relevant)': 'dealStage',
  'deal stage(if relevant)': 'dealStage',
  'dealstage': 'dealStage',
  'stage': 'dealStage',
...
// Handles: "Relevant", "Revelant" (common typo in sheet), "Yes", "Qualified", "Good", "Hot", "1", "true"
function parseRelevant(raw) {
  if (!raw) return false;
  const v = raw.trim().toLowerCase();
  return v === 'relevant' || v === 'revelant' || v === 'yes' || v === 'qualified'
      || v === 'good' || v === 'hot' || v === '1' || v === 'true';
}

// A lead that reached any active/positive pipeline stage is implicitly relevant,
// even if the Lead Quality cell wasn't filled in.
function isPositiveStage(stage) {
  return isDealClosed(stage) || isInDiscussions(stage) || isTourMeeting(stage)
      || isFutureSale(stage) || /quotation/i.test(stage || '');
}

// Normalize Lead Source typos/variants → canonical channel label
//   "Meta Form" / "Meta from" / "Meta form" → "Meta Form"
//   "Website - Private Offices" / "Website" → "Website"
//   "DMs" / "DM" → "DMs"
function normalizeSource(raw) {
  const s = (raw || '').trim();
  if (!s) return null;
  const low = s.toLowerCase();
  if (low.startsWith('meta'))    return 'Meta Form';
  if (low.startsWith('website')) return 'Website';
  if (/^dms?$/.test(low))        return 'DMs';
  return s;
}
...
export function parseLeadsFromRows(rows) {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    const dealStage = (row.dealStage || '').trim() || null;
    ...
    leads.push({
      ...
      contactedBy: (row.contactedBy || '').trim() || null,
      contactStatus: (row.contactStatus || '').trim() || null,
      // Relevant = explicitly marked relevant OR reached a positive pipeline stage
      relevant: parseRelevant(row.relevant) || isPositiveStage(dealStage),
      leadQuality: (row.relevant || '').trim() || null,  // raw Lead Quality value
      dealStage,
...
// ─── Google Ads tab parser ─────────────────────────────────────────────────
// Parses the "Google Ads Data" tab written by the Google Ads Script.
// Expected headers (row 1): Date | Spend (EGP) | Clicks | Impressions | CTR% | Avg CPC (EGP) | Conversions

export function parseSpendFromRows(rows) {
  ...
  if (dateIdx === -1 || spendIdx === -1) return [];
  ...
  return rows.slice(1)
    .map(row => {
      ...
      return {
        date,
        google:      parseFloat((row[spendIdx]       || '0').replace(/[,\s]/g, '')) || 0,
        clicks:      parseInt  ((row[clicksIdx]      || '0').replace(/[,\s]/g, '')) || 0,
        impressions: parseInt  ((row[impressionsIdx] || '0').replace(/[,\s]/g, '')) || 0,
        conversions: parseFloat((row[conversionsIdx] || '0').replace(/[,\s]/g, '')) || 0,
        meta: null,
      };
    })
    .filter(Boolean)
